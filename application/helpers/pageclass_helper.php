...
function show($mode=1)
{
  switch ($mode)
  {
   ...
   case '6':
    ...
    $this->first_page='首页';
    $this->last_page='尾页';
    return $this->first_page().$this->pre_page().$this->nowbar('','hver').$this->next_page().$this->last_page();
    break;
   case '7':
    $this->format_left='';
    $this->format_right='';    
    ...
    $this->first_page='首页';
    $this->last_page='尾页';
    $pagestr = "<a style='border: 1px solid #000000;'>";
    $pagestr .= "共" . $this->totalresult . "个";
    $pagestr .= "/"; 
    $pagestr .= $this->totalpage . "页";
    $pagestr .= "</a>";
    return $pagestr . $this->first_page("first paginate_button paginate_button_disabled").$this->pre_page("previous paginate_button paginate_button_disabled").$this->nowbar('paginate_button','on').$this->next_page("next paginate_button").$this->last_page("last paginate_button");
    break;   
  case '8':
  ...
  case '9':
    $this->next_page='下一页';
    $this->pre_page='上一页';
    return $this->pre_page("ppre").$this->next_page("pnext");
    break;
    case '10':
    $this->next_page='';
    $this->pre_page='';
    return $this->pre_page("pre").$this->next_page("next");
    break;
  }
...
function _set_url($url="")
{
  if(!empty($url)){
   ...
   //$this->url=$url.((stristr($url,'?'))?'&':'?').$this->page_name."=";
      $this->url = str_replace('/index.php', '', $_SERVER['REQUEST_URI'])."?".$this->page_name."="; 
  }else{
      //自动获取
   if(empty($_SERVER['QUERY_STRING'])){
    ...
    $this->url = str_replace('/index.php', '', $_SERVER['REQUEST_URI'])."?".$this->page_name."="; 
      ...
      // echo $this->url.'<br/>'.$_SERVER['QUERY_STRING'];
