<?php
class LibresourceController extends CI_Controller {
    ...
    */
    function setComponent( $view, $data = false ) {
        ...
        $this->_component = $this->load->view( "components/libresource/" . $view, $data, true );
    }
    ...
    */
    function showTemplate( $templates, $title = false, $keywords = false, $description = false ) {
        ...
        $this->setModule( 'libresource_menus', array('topMenus' => $topMenus, 'currentTopMenuId' => $currentTopMenuId, 'user' => $this->user) );
        //左侧导航视图
        $left_menus = $this->Front_Power_Menu_Model->getAll("`f_id` = '".$currentTopMenuId."'", "*", "`order` ASC");
        $this->setModule( 'libresource_left', array("left_menus" => $left_menus, "currentLeftMenuId" => $currentLeftMenuId, 'user' => $this->user) );
