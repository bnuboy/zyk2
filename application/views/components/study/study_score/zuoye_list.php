...
<div class="noticesbox" style="width:484px;">
    <div class="noticewarp" style="width:484px;">
        ...
        <div class="noticekatebox" style="width:484px;">
            <div class="notiness">共有作业<span><?= $count ?> 个</span></div>
        </div>
                            ...
                            { ?>
                                <tr>
                                    <td><input type="checkbox" name ="item_id[]" value="<?= $val[ 'id' ] ?>" 
                                               onchange="oncheckbox(this,<?= $val[ 'id' ] ?>,'<?= $val[ 'title' ] ?>')" id='cat_cb_<?= $val[ 'id' ] ?>'
                                               <?php
                                               if ( !empty( $ids ) )
                                               {
...
<?php } ?>
            ...
            <input name="" type="button" value="取消" class="remove" onclick="hite();"/>

            <!--$pagination-->
            <div id='selected_cat'>
                <?php if(!empty ($class_attr)){
    foreach ($class_attr as $key=>$val){
       echo  ' <label cat_name="'.$val["title"].'" cat_id="'.$val["id"].'"><span onClick="deleteselect('.$val["id"].')">×</span>'.$val["title"].'</label>';
    }  
                }?>
            </div>
        </div>
