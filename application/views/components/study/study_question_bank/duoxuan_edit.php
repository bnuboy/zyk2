...
<script>
     ...
     function addsubmit(){
        ...
        var check_count = $("#answer_id [input type='checkbox']:checked").length;
        if(count ==0){
            alert('至少要有一个选项内容');
            return false;
        }
     ...
     function addSubject()
    {
        ...
        <a href="javascript:;" onclick="delete_item_adjust(this)">删除</a></td></tr>' );
                $("#answer_id").append("<span><input name='daan[]' value="+count+" type='checkbox'/>"+abc_alias[count]+"</span>");
            }
            function delete_item_adjust( obj ){
                ...
                item.remove();
                $("#answer_id span").remove();
                $.each($("#subject_id table tr"),function(key,obj){
                    $(obj).find('span').text( abc_alias[key] + ":" );
                    $("#answer_id").append("<span><input name='daan[]' value='"+key+"' type='checkbox' />"+abc_alias[key]+"</span>");
                });
            }
    </script>
<!--管理信息-->
<div class="noticesbox" id="child_2" >
    <div class="noticewarp">
        ...
        <div class="noticenwarp">
            <form action="/study_question_bank/edit_duoxuan/<?=$patterntype_id?>/<?=$info['id']?>" method="post" id="dataform" onsubmit="return addsubmit();">
                ...
                <div class="noticekatebox">
                    ...
                    <div class="addfile" style="width:580px;height: auto;" id="subject_id">
                        <table>
                ...
                <div class="noticekatebox">
                    ...
                    <div class="scselects" id="answer_id">   
                       
                        <?php foreach (unserialize($info['xx']) as $key => $val) {?>
                        <span>  
                            <input name="daan[]" type="checkbox" value="<?=$key?>" <?php if(strstr($info['daan'],(string)$key)) echo 'checked';?>/>
                            <?=$abc_array[$key]?></span>
                        <?php }?>
                    </div>
                </div>
