...
<div class="Stc w400">
  ...
  <div class="Stc-bottom zcjg">
      <form action="/study_stu_question/tiwen_add" method="post">
    <h3>你是否想要想要问：</h3>
   <ul class="tw_list">
       <?php if(!empty($list)){?>
       <?php foreach($list as $key=>$val){?>
       <li><a href="/study_stu_question/see_question/<?=$val['id']?>" class="iframe"><?=$val['title']?></a>    &nbsp;&nbsp;&nbsp;          <?=$val['count']?>个回答      <?=!empty($val['last_riqi']['last_time']) ? $val['last_riqi']['last_time'] : '0000:00:00 00:00:00';?> </li>
       <?php }?>
       <?php }else{?>
            没有类似的问题
       <?php } ?>
