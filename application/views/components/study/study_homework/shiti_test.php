      ...
      <div class="noticesbox">
        <div class="noticewarp tea-cont">
          <div class="noticetit tearch-nav">
            ...
            <div><a href="/study_homework/index" class="blue">&lt;&lt;返回</a></div>
                            ...
                            <span style="margin-left: 10px; display: block;margin-top: 10px;margin-bottom: 10px;">
                               <h4><?=$k+1?>.<?=$a['title']?></h4>          
                                ...
                                <?php  foreach ($a['xx'] as $ks=>$vs){?>
                                     <li  style="line-height:25px;"><?php if($key=='danxuan'){?><input type="radio" name="danxuan[<?=$attr[1]?>][<?=$a['id']?>]" value="<?=$ks?>" /><?php }else{?>
                                         <input type="checkbox" name="duoxuan[<?=$attr[1]?>][<?=$a['id']?>][]" value="<?=$ks?>" /><?php }?>
                                         <?=$abc_array[$ks]?>. <?=$vs?></li>
                                <?php }?>                           
                                   </ul>
                             </span>
                            ...
                            <h4><?=$k+1?>.<?=$a['title']?></h4>          
                                ...
                                <?php  foreach ($a['timu'] as $ks=>$vs){?>
                                    ...
                                    <?php  foreach($vs as $kss=>$vss){?>
                                        <input type="radio" name="wanxingtiankong[<?=$attr[1]?>][<?=$a['id']?>][<?=$ks?>]" value="<?=$kss?>" />
                                        <?=$abc_array[$kss]?>. <?=$vss?>&nbsp;&nbsp;
                                    <?php }?>
                                   </li>
                                <?php }?>                           
                                ...
                                <ul style="margin-left: 25px;display: block;"> 
                                    <li  style="line-height:25px;">                           
                                        <?php  foreach($a['xuanxiang'][$ks] as $ki=>$vi ){?>
                                            <input type="radio" name="yuedulijie[<?=$attr[1]?>][<?=$a['id']?>][<?=$ks?>]" value="<?=$ki?>" />
                                            <?=$abc_array[$ki]?>.<?=$vi?>&nbsp;&nbsp;
                                        <?php }?> 
                                    </li>                  
                                </ul>
