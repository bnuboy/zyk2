...
<script>
   $().ready(function() {
      ...
      $("#sub_form").validate({
          ...
          messages: {
              "data[name]": {required : "请填写大赛名称"},
              "data[course_uuid]": {required : "请填写大赛代码"},
              "data[organization_id]": {required : "请选择所属院系"},
              "data[classify_cat_id]": {required : "请选择大赛分类"},
...
<form action="/ucenter_course/courseedit" enctype="multipart/form-data" method="post" id="sub_form" > 
  <div class="noticewarp" >

    <div class="noticetit">
      <h1>我的大赛-<?php echo !empty($data['id'])?"编辑":"新增"; ?></h1>
    </div>
    ...
    <div class="noticenwarp">
      ...
      <table>
        <tr colspan='3'>
          <td class="maddpword">大赛名称:</td> 
          <td class="maddness" style="width:400px; float:none"> 
      ...
      <div class="noticekatebox" style="width:800px;height:58px;">
        ...
        <div  class="maddness"  style="width:600px;height:58px;float:none">
          <input name="data[img]" id="img" type="hidden" value="<?=isset($data['img']) ? $data['img'] : ''; ?>"/>
          <iframe style="border:0px; margin-top:-9px;" src="/Uploadfiles/uploadfileform?fileid=img&defaultvalue=<?php echo empty($data['img']) ? '' : $data['img']?>&allowed_extensions=gif|jpg|sql&overwrite=false&encrypt_name=false" scrolling="no" width="400px" height="54px;"></iframe>
        </div>
      </div>
      ...
      <div class="basebutbox">
       ...
       <div class="addbutin"><input type="submit" class="addbut" value="保存" id="submit" onClick="dianji();"/></div>
     </div>
    </div>
...
<script>
    ...
    function user_select_checkbox(){
        ...
        var where      = "`type` = 'teacher' AND `enabled` = 'y' AND `id` <>"+user_id;
